...
export const SITE_ORIGIN = "https://redskyg.com";
export const SITE_BASE_PATH = "/landing/";
...
export const LOCALE_CONFIG = {
  "es-mx": {
    ...
    footer: {
      ...
      officeAddress:
        "San Nicolás Tolentino, Toluca de Lerdo, Estado de México, México",
      customerService: "Atención a clientes",
      mail: "Correo",
      newsletterTitle: "Recibe novedades de aviación",
      newsletterCopy:
  ...
  "en-us": {
    ...
    footer: {
      ...
      office: "Main office",
      officeAddress:
        "San Nicolás Tolentino, Toluca de Lerdo, State of Mexico, Mexico",
      customerService: "Customer service",
      mail: "Mail",
      newsletterTitle: "Get the Latest Aviation Updates",
      newsletterCopy:
...
export function getLocalizedPath(locale, pageKey = "home") {
  const normalizedLocale = normalizeLocale(locale);
  const segment = PAGE_SEGMENTS[pageKey] ?? "";
  return segment ? `/${normalizedLocale}/${segment}` : `/${normalizedLocale}`;
}
...
export function detectPreferredLocale() {
  ...
  for (const browserLocale of browserLocales) {
    ...
    const region = lowerLocale.split("-")[1]?.toUpperCase();
...
export function buildAbsoluteUrl(path) {
  ...
  return new URL(normalizedPath, `${SITE_ORIGIN}${normalizedBase}`).toString();
}
...
export function getSeoForRoute(route) {
  ...
  const title =
    pageKey === "home"
      ? localeConfig.seo.homeTitle
      : pageMeta?.title ?? localeConfig.seo.defaultTitle;

  const description =
    pageKey === "home"
      ? localeConfig.seo.homeDescription
      : pageMeta?.description ?? localeConfig.seo.defaultDescription;
  ...
  alternates.push({
    hreflang: "x-default",
    href: buildAbsoluteUrl(`/${DEFAULT_LOCALE}`),
  });
  ...
  return {
    ...
    siteName: localeConfig.seo.siteName,
    robots: shouldIndex
